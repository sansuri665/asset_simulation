...
class InstitutionOrganizationTests(unittest.TestCase):
    def test_current_institution_is_ten_million_dollar_prop_firm(self) -> None:
        ...
        self.assertEqual(10_000_000.0, config["capital_base"]["initial_proprietary_capital_usd"])
        self.assertFalse(config["capital_base"]["external_aum_enabled"])
        self.assertFalse(config["capital_base"]["fund_management_company_split_enabled"])
        self.assertEqual("institution_organization_v1", contract["contract_id"])
        self.assertEqual("institution_organization_v1", config["capital_base"]["runtime_capital_owner"])
        self.assertFalse(config["capital_base"]["market_capacity_binding_expected_at_initial_scale"])
    ...
    def test_strategy_compatibility_capital_cannot_drift_from_owner(self) -> None:
        ...
        self.assertEqual(initial_proprietary_capital_usd(assets), validate_strategy_capital_reference(strategy, assets=assets))
    ...
    def test_registered_organization_assets_validate_as_one_contract(self) -> None:
        ...
        self.assertEqual(contract["department_ids"], [item["department_id"] for item in config["departments"]])

    def test_investment_decision_is_explicit_governance_not_department(self) -> None:
        ...
        self.assertEqual(100.0, decision["single_strategy_default_capital_authorization_pct"])
        self.assertEqual(
            {
        ...
        self.assertTrue(decision["strategy_charter_enabled"])
        self.assertTrue(decision["company_risk_appetite_enabled"])
        self.assertTrue(decision["position_mandate_enabled"])
    ...
    def test_legacy_runtime_proxy_remains_full_single_strategy_until_cutover(self) -> None:
        assets = load_registered_assets()
        proxy = assets["oil_strategy_risk_config"]["committee_proxy"]
        self.assertEqual(100.0, proxy["default_capital_authorization_pct_of_company_equity"])
        self.assertEqual("single_strategy_full_allocation_proxy", proxy["capital_authorization_method"])

    def test_administration_is_shell_only(self) -> None:
        config = load_registered_assets()["institution_organization_config"]
        departments = {item["department_id"]: item for item in config["departments"]}
        self.assertEqual(
            {"forecast_research", "investment_strategy", "corporate_risk", "trading_execution", "administration"},
            set(departments),
        )
