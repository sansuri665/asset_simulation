...
from asset_simulation.model.oil_shipping_regions import (
...
class NorthAmericaImportCycleTests(unittest.TestCase):
    def test_refinery_cycle_is_persistent_bounded_and_nontrivial(self) -> None:
        ...
        for turn_index in range(10 * 12):
            state, target = _advance_north_america_import_cycle(
                state,
                seed=42,
        ...
        self.assertGreater(statistics.stdev(monthly_changes), 0.025)
        self.assertLess(statistics.stdev(monthly_changes), 0.14)
        self.assertGreater(max(adjustments) - min(adjustments), 0.22)
        self.assertGreaterEqual(min(targets), -0.50 - 1e-8)
        self.assertLessEqual(max(targets), 0.28 + 1e-8)
        self.assertLessEqual(max(map(abs, monthly_changes)), 0.14 + 1e-8)

    def test_north_america_imports_have_own_later_spring_autumn_cycle(self) -> None:
        ...
        for turn in turns:
            ...
            north_america = regions["north_america_import"]
            self.assertLess(float(north_america["net_seaborne_balance_mbd"]), 0.0)
            self.assertAlmostEqual(
                float(north_america["production_policy_adjustment_mbd"]),
                0.0,
                places=8,
            )
            self.assertAlmostEqual(
                float(north_america["production_cycle_adjustment_mbd"]),
                0.0,
                places=8,
            )
            ...
            imports.append(-float(north_america["net_seaborne_balance_mbd"]))
            refinery_adjustments.append(
                float(north_america["refinery_cycle_adjustment_mbd"])
            )
        ...
        self.assertGreater(statistics.stdev(monthly_changes), 0.050)
        self.assertLess(statistics.stdev(monthly_changes), 0.14)
        self.assertGreater(max(imports) - min(imports), 0.28)
        self.assertGreater(
