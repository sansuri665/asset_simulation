"""Compact physical oil balance and light commodity-price block."""
...
def annual_price_envelope(
    *,
    ...
    """Year-range envelope around a settled annual open/close.

    Close remains the year-end real oil index. High and low are leftover
    intra-year path width, not a second close and not traded OHLC.
    Tight markets widen the range. The split follows the annual drift:
    up years keep most leftover below the close, down years above it, so
    trend years finish near the extreme instead of looking like rejections.
    Always ``low <= min(open, close) <= max(open, close) <= high``.
    """
...
def expand_envelope_to_steps(
    *,
    ...
    """Expand one OHLC envelope into chained intra-envelope bars.

    Closes follow the parent open-to-close path with shocks, so bodies are
    not an even split of the parent move. Closes are not pinned to the
    parent high/low; leftover range is swept by wicks. Quiet bars add extra
    upper and lower wicks. This is not a traded market.
    """
    ...
    leftover_dn = min(open_px, close_px) - low_px
    corridor_fraction = 0.97 if steps <= 4 else 0.90
    hi_cap = high_px if leftover_up <= 1e-12 else max(open_px, close_px) + corridor_fraction * leftover_up
    lo_cap = low_px if leftover_dn <= 1e-12 else min(open_px, close_px) - corridor_fraction * leftover_dn
    log_open = math.log(open_px)
    log_close = math.log(close_px)
    trend_step = (log_close - log_open) / steps
    shocks = []
    for step in range(1, steps + 1):
        raw_shock = shock_scale * normal(seed, f"{stream_prefix}_step_{step}", year_index)
    ...
    # Reflect bridge overshoots back into the close corridor.  The previous
    # point-by-point clamp could pin several consecutive closes to exactly
    # the same cap, creating artificial monthly/weekly plateaus. Reflection
    # preserves the endpoint and most of the stochastic movement without
    # allowing a run of bars to stick to a hard boundary.
    log_hi_cap = math.log(hi_cap)
    log_lo_cap = math.log(lo_cap)
    corridor_span = log_hi_cap - log_lo_cap
    if corridor_span <= 1e-12:
        reflected_logs = [log_open] * (steps + 1)
    else:
        reflected_logs = []
        for raw_log in raw_logs:
            offset = (raw_log - log_lo_cap) % (2.0 * corridor_span)
            reflected_logs.append(
                log_lo_cap + offset
                if offset <= corridor_span
                else log_hi_cap - (offset - corridor_span)
            )
    ...
    for step in range(1, steps + 1):
        open_m = prices[step - 1]
        close_m = prices[step]
        signed_body_log = 100.0 * math.log(close_m / open_m) if open_m > 0.0 else 0.0
        body_log = abs(signed_body_log)
        body_high = max(open_m, close_m)
        body_low = min(open_m, close_m)
        quiet_frac = clamp((quiet_body - body_log) / quiet_body, 0.0, 1.0) if quiet_body > 0.0 else 0.0
        wick_budget = clamp(0.48 * body_log + 2.8, 2.0, 12.0)
        split = clamp(
            0.50
            - 0.34 * math.tanh(signed_body_log / 9.0) * (1.0 - 0.55 * quiet_frac)
            + 0.08 * normal(seed, f"{stream_prefix}_wick_{step}", year_index),
            0.12 + 0.18 * quiet_frac,
            0.88 - 0.18 * quiet_frac,
        )
        quiet_span = quiet_span_scale * quiet_frac
        quiet_tilt = 0.22 * normal(seed, f"{stream_prefix}_quiet_wick_{step}", year_index)
        upper_log = wick_budget * split + quiet_span * clamp(1.0 + quiet_tilt, 0.55, 1.45)
        lower_log = wick_budget * (1.0 - split) + quiet_span * clamp(1.0 - quiet_tilt, 0.55, 1.45)
        high_m = min(high_px, body_high * math.exp(upper_log / 100.0))
        low_m = max(low_px, body_low * math.exp(-lower_log / 100.0))
        bars.append(
            {
                step_field: float(step),
                "open": round(open_m, 8),
                "high": round(max(high_m, body_high), 8),
                "low": round(min(low_m, body_low), 8),
                "close": round(close_m, 8),
            }
        )

    high_bar = max(range(steps), key=lambda i: max(bars[i]["open"], bars[i]["close"]))
    low_bar = min(range(steps), key=lambda i: min(bars[i]["open"], bars[i]["close"]))
    if high_bar == low_bar:
        others = [i for i in range(steps) if i != high_bar]
        if leftover_up >= leftover_dn:
            low_bar = min(others, key=lambda i: min(bars[i]["open"], bars[i]["close"]))
        else:
            high_bar = max(others, key=lambda i: max(bars[i]["open"], bars[i]["close"]))
...
def expand_annual_to_months(
    *,
    ...
    """Expand one annual envelope into 12 intra-year bars.

    Monthly closes follow the annual open-to-close path with intra-year
    shocks, so typical bodies are not a 12-way split of the year. Closes are
    not pinned to the year high/low; leftover envelope is swept by wicks.
    Quiet months add extra upper and lower wicks so a small body is not a
    bald doji. Up months keep more leftover below the close. This is not a
    traded monthly market and does not write back the annual close.
    """
...
def expand_month_to_weeks(
    *,
    ...
    """Expand one monthly envelope into 4 intra-month bars.

    Uses a distinct week stream so monthly OHLC stays prefix-stable. Week 1
    open equals the month open; week 4 close equals the month close. Month
    high/low are tagged by weekly wicks, not by pinning a weekly close.
    """
    ...
    return expand_envelope_to_steps(
        ...
        low_px=low_px,
        steps=4,
        shock_scale=0.076 * clamp(float(volatility_regime_index), 0.75, 1.40),
        max_step=0.115,
        stream_prefix=f"oil_week_{int(month)}",
        step_field="week",
        quiet_body=1.8,
        quiet_span_scale=2.4,
    )
