...
MODEL_VERSION = "asset-simulation-gulf-east-asia-single-route-pricing-v0.2.1"
...
def shipping_turn_days(month_days: int) -> tuple[int, int, int]:
    ...
    _positive_integer(month_days, "month_days")
...
def _route_cargo_mbd(month: Mapping[str, Any], route_id: str) -> float:
    ...
    cargo_mbd = _finite(route["cargo_mbd"], "route cargo")
    if cargo_mbd < 0.0:
...
def price_single_route_turn(
    *,
    ...
    cargo_mbd = _finite(structural_cargo_mbd, "structural cargo")
    ...
    structural_cargo = _finite(cargo_mbd * days, "turn cargo")

    inventory_cfg = market["inventory"]
    inventory_gap = _finite(0.5 * origin - 0.5 * destination, "inventory gap")
    ...
    previous_tce = (
        ...
        else _finite(previous_real_tce_2025_usd_per_day, "previous TCE")
    )
    if previous_tce <= 0.0:
    ...
    # Clamp before exponentiation as well as at the published-price guard.
    ...
    nominal_tce = _finite(real_tce * (cpi / 100.0), "nominal TCE")
...
def build_lagged_prompt_supply_path(
    ...
    *,
    ...
    market = load_single_route_pricing_config() if config is None else dict(config)
    _validate_config(market)
    supply_multiplier = _finite(supply_multiplier, "supply multiplier")
    if supply_multiplier <= 0.0:
        raise ValueError("supply multiplier must be positive")
    if isinstance(lag_turns, bool) or not isinstance(lag_turns, int) or lag_turns < 0:
...
def simulate_gulf_east_asia_price_path(
    ...
    *,
    ...
    for month in months:
        ...
        cpi = _finite(cpi_by_year[year], "CPI")
        ...
        for turn_in_month, turn_days in enumerate(
            ...
            prompt_supply = _finite(prompt_supply_by_turn[turn_index], "prompt supply")
    ...
    summary = {
        ...
        # Compatibility alias: this is a sum of turn observations, NOT backlog.
    ...
    summary["cumulative_unfilled_fixture_observations_vlcc"] = summary["total_unfilled_fixture_vlcc"]
