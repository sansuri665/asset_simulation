...
OIL_SHORT_HORIZON_RISK_MODEL_VERSION = "asset-simulation-oil-short-horizon-risk-v0.1.0"
...
def _piecewise(anchor: Mapping[str, Any], score: float) -> float:
    ...
    if value <= 50.0:
        mix = value / 50.0
        return float(anchor["score_0"]) + mix * (float(anchor["score_50"]) - float(anchor["score_0"]))
    mix = (value - 50.0) / 50.0
    return float(anchor["score_50"]) + mix * (float(anchor["score_100"]) - float(anchor["score_50"]))


def _capability_error(anchor: Mapping[str, Any], score: float) -> float:
    value = clamp(float(score), 35.0, 70.0)
    mix = (value - 35.0) / 35.0
    return float(anchor["score_35"]) + mix * (float(anchor["score_70"]) - float(anchor["score_35"]))
...
def _pack_profile(
    *,
    ...
    capabilities = {key: _score(capability_radar[key], key) for key in capability_dimensions}
    result = {
...
def generate_oil_short_horizon_risk_candidate(*, seed: int, candidate_index: int) -> dict[str, Any]:
    ...
    style_latents = {
        key: clamp(normal(seed, f"oil_short_risk.{candidate_index}.style.{key}", candidate_index), -1.8, 1.8)
        for key in generation["style_latent_traits"]
    }
    capability_latents = {
        key: clamp(normal(seed, f"oil_short_risk.{candidate_index}.capability.{key}", candidate_index), -1.5, 1.5)
    ...
    for index, dimension in enumerate(config["style_dimensions"]):
        ...
        value = float(generation["style_center"]) + sum(float(loading[key]) * style_latents[key] for key in style_latents)
        value += float(generation["style_idiosyncratic_scale"]) * normal(seed, f"oil_short_risk.{candidate_index}.style_dimension.{dimension}", index)
        styles[dimension] = round(clamp(value, float(generation["style_floor"]), float(generation["style_ceiling"])), 2)
    ...
    for index, dimension in enumerate(config["capability_dimensions"]):
        ...
        value = float(generation["capability_center"]) + sum(float(loading[key]) * capability_latents[key] for key in capability_latents)
        value += float(generation["capability_idiosyncratic_scale"]) * normal(seed, f"oil_short_risk.{candidate_index}.capability_dimension.{dimension}", index)
        capabilities[dimension] = round(clamp(value, float(generation["capability_floor"]), float(generation["capability_ceiling"])), 2)
    families = list(generation["family_names"])
    names = list(generation["given_names"])
    family_index = int(abs(normal(seed, f"oil_short_risk.{candidate_index}.family", candidate_index)) * 1000) % len(families)
    name_index = int(abs(normal(seed, f"oil_short_risk.{candidate_index}.name", candidate_index)) * 1000) % len(names)
    return _pack_profile(
...
def generate_oil_short_horizon_risk_roster(*, seed: int, candidate_count: int | None = None) -> dict[str, Any]:
    assets, config, contract = _assets()
    generation = config["candidate_generation"]
    ...
    candidates = [generate_oil_short_horizon_risk_candidate(seed=seed, candidate_index=index) for index in range(count)]
    result = {
...
def resolve_oil_short_horizon_risk_profile(profile: Mapping[str, Any] | None) -> dict[str, Any]:
    if profile is None:
    ...
    if supplied.get("profile_hash") is not None and supplied["profile_hash"] != rebuilt["profile_hash"]:
...
def _visible_weeks(contract: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    return [week for month in contract.get("monthly", ()) for week in month.get("weekly", ())]
...
def _visible_annualized_volatility(contract: Mapping[str, Any], config: Mapping[str, Any]) -> float:
    ...
    closes = [float(item["close"]) for item in _visible_weeks(contract)[-(lookback + 1):] if float(item["close"]) > 0.0]
    ...
    if not returns:
        ...
        realized = math.sqrt(float(config["visible_volatility"]["annualization_weeks"]) * sum(value * value for value in returns) / len(returns))
    return clamp(realized, float(config["visible_volatility"]["minimum_annualized_volatility"]), float(config["visible_volatility"]["maximum_annualized_volatility"]))


def _spread_change_volatility(first: Mapping[str, Any], second: Mapping[str, Any], config: Mapping[str, Any]) -> float:
    lookback = int(config["calendar_spread"]["spread_change_lookback_weeks"])
    left = {int(item["week_serial"]): float(item["close"]) for item in _visible_weeks(first)}
    right = {int(item["week_serial"]): float(item["close"]) for item in _visible_weeks(second)}
...
def _company_policy(risk_appetite: Mapping[str, Any], profile: Mapping[str, Any], config: Mapping[str, Any]) -> dict[str, float]:
    ...
    radar = {key: float(value) for key, value in risk_appetite["risk_appetite_radar"].items()}
    mapping = config["risk_appetite_parameter_mapping"]
    style = profile["style_radar"]
    style_mapping = config["style_parameter_mapping"]
    intervention = min(1.0, _piecewise(style_mapping["intervention_limit_multiplier"], style["intervention_earliness"]))
    monitoring_error = _capability_error(config["capability_error_mapping"]["monitoring_abs_error"], profile["capability_radar"]["monitoring_discipline"])
    monitoring_noise = _signed_error(profile["profile_hash"], "monitoring_limit") * monitoring_error
    intervention = min(1.0, max(0.5, intervention * (1.0 + monitoring_noise)))
    return {
        "max_strategy_stress_loss_pct_of_allocated_capital": _piecewise(mapping["max_strategy_stress_loss_pct_of_allocated_capital"], radar["strategy_stress_loss_tolerance"]) * intervention,
        "max_company_stress_loss_pct_of_equity_per_strategy": _piecewise(mapping["max_company_stress_loss_pct_of_equity_per_strategy"], radar["company_materiality_tolerance"]) * intervention,
        "max_margin_pct_of_allocated_capital": _piecewise(mapping["max_margin_pct_of_allocated_capital"], radar["margin_tolerance"]) * intervention,
        "max_company_margin_pct_of_equity_per_strategy": _piecewise(mapping["max_company_margin_pct_of_equity_per_strategy"], radar["margin_tolerance"]) * intervention,
        "max_single_contract_share_of_market_gross": _piecewise(mapping["max_single_contract_share_of_market_gross"], radar["concentration_tolerance"]) * min(1.0, _piecewise(style_mapping["concentration_limit_multiplier"], style["concentration_aversion"])),
        "max_liquidation_half_turns": _piecewise(mapping["max_liquidation_half_turns"], radar["liquidity_tolerance"]) * min(1.0, _piecewise(style_mapping["liquidity_limit_multiplier"], style["liquidity_priority"])),
        "roll_buffer_half_turns": _piecewise(mapping["roll_buffer_half_turns"], radar["roll_tolerance"]) + 2.0 * float(style["intervention_earliness"]) / 100.0,
        "tail_stress_multiplier": _piecewise(style_mapping["tail_stress_multiplier"], style["tail_risk_focus"]),
        "model_uncertainty_multiplier": _piecewise(style_mapping["model_uncertainty_multiplier"], style["model_skepticism"]),
    }
...
def _risk_estimates(
    *,
    ...
    measurement_amp = _capability_error(config["capability_error_mapping"]["risk_measurement_abs_error"], profile["capability_radar"]["risk_measurement"])
    stress_amp = _capability_error(config["capability_error_mapping"]["stress_analysis_abs_error"], profile["capability_radar"]["stress_analysis"])
    per_contract: dict[str, Any] = {}
    margin = 0.0
    directional_stress = 0.0
    as_of = market.get("asOf", {})
    for contract_id, target in targets.items():
        contract = contracts.get(contract_id)
        if contract is None:
            per_contract[contract_id] = {"target_lots": target, "contract_available": False}
            continue
        price = float(contract["price_usd"])
        observed_vol = _visible_annualized_volatility(contract, config)
        measurement_error = _signed_error(profile["profile_hash"], contract_id, as_of, "vol") * measurement_amp
        estimated_vol = max(0.0, observed_vol * (1.0 + measurement_error))
        contract_margin = abs(target) * price * contract_size * initial_margin_rate
        stress = abs(target) * price * contract_size * estimated_vol
        margin += contract_margin
        directional_stress += stress
        per_contract[contract_id] = {
            "target_lots": target,
            "contract_available": True,
            "price_usd": price,
            "visible_annualized_volatility": observed_vol,
            "estimated_annualized_volatility": estimated_vol,
            "measurement_error_fraction": measurement_error,
            "initial_margin_usd": contract_margin,
            "directional_one_sigma_proxy_usd": stress,
        }
    base_stress = directional_stress
    spread_report: dict[str, Any] | None = None
    active = [(key, value) for key, value in targets.items() if value != 0 and key in contracts]
    if strategy_type == "calendar_spread" and len(active) == 2:
        (first_id, first_target), (second_id, second_target) = active
        if first_target == -second_target and abs(first_target) == abs(second_target):
            spread_vol = _spread_change_volatility(contracts[first_id], contracts[second_id], config)
            stressed_move = max(float(config["calendar_spread"]["minimum_stressed_spread_move_usd_per_bbl"]), spread_vol * policy["tail_stress_multiplier"] * policy["model_uncertainty_multiplier"])
            base_stress = abs(first_target) * contract_size * stressed_move
            spread_report = {
                "balanced_pair": True,
                "spread_units": abs(first_target),
                "visible_spread_change_volatility_usd_per_bbl": spread_vol,
                "stressed_spread_move_usd_per_bbl": stressed_move,
            }
    stress_error = _signed_error(profile["profile_hash"], strategy_type, as_of, "stress") * stress_amp
    estimated_stress = max(0.0, base_stress * policy["tail_stress_multiplier"] * policy["model_uncertainty_multiplier"] * (1.0 + stress_error))
    return {
        "per_contract": per_contract,
        "initial_margin_usd": margin,
        "base_risk_proxy_usd": base_stress,
...
def build_oil_short_horizon_risk_review(
    ...
    *,
    ...
    assets, config, contract = _assets()
    if committee_position_mandate.get("identity", {}).get("object_type") != "position_mandate":
        raise ValueError("oil short-horizon risk requires an actual committee position mandate")
    ...
    if not math.isfinite(equity) or equity <= 0.0 or not math.isfinite(allocated) or allocated <= 0.0:
        raise ValueError("company equity and allocated strategy capital must be positive")
    if not math.isclose(allocated, float(committee_position_mandate["authorized_strategy_capital_usd"]), rel_tol=0.0, abs_tol=1e-6):
        raise ValueError("risk capital input must match the committee capital mandate")
    expected = {str(key): int(value) for key, value in committee_position_mandate["committee_expected_targets"].items()}
    if any(isinstance(value, bool) for value in committee_position_mandate["committee_expected_targets"].values()):
        raise ValueError("committee expected targets must be integer lots")
    positions = {str(key): int(value) for key, value in dict(current_positions or {}).items()}
    profile = resolve_oil_short_horizon_risk_profile(risk_profile)
    policy = _company_policy(company_risk_appetite, profile, config)
    contracts = {str(item["contract_id"]): item for item in market["curve"]["contracts"]}
    market_gross_cap = int(market["participantLimitsPolicy"]["all_contract_gross_position_cap_lots"])
    ...
    for contract_id, target in expected.items():
        ...
        if contract_item is None:
            cap = 0
            hard_facts[contract_id] = {"contract_available": False, "current_position_lots": current, "committee_expected_target_lots": target}
            ...
            concentration_cap = math.floor(market_gross_cap * policy["max_single_contract_share_of_market_gross"])
            liquidity_cap = math.floor(turn_limit * policy["max_liquidation_half_turns"])
        ...
        approved = int(math.copysign(min(abs(target), max(0, cap)), target)) if target else 0
    ...
    pre_estimates = _risk_estimates(market=market, targets=preliminary, strategy_type=strategy_type, profile=profile, policy=policy, config=config)
    stress = float(pre_estimates["estimated_stress_loss_usd"])
    margin = float(pre_estimates["initial_margin_usd"])
    stress_strategy_limit = allocated * policy["max_strategy_stress_loss_pct_of_allocated_capital"] / 100.0
    stress_company_limit = equity * policy["max_company_stress_loss_pct_of_equity_per_strategy"] / 100.0
    margin_strategy_limit = allocated * policy["max_margin_pct_of_allocated_capital"] / 100.0
    margin_company_limit = equity * policy["max_company_margin_pct_of_equity_per_strategy"] / 100.0
    scale_candidates = {
        "strategy_stress": 1.0 if stress <= 1e-12 else stress_strategy_limit / stress,
        "company_materiality": 1.0 if stress <= 1e-12 else stress_company_limit / stress,
        "strategy_margin": 1.0 if margin <= 1e-12 else margin_strategy_limit / margin,
        "company_margin_materiality": 1.0 if margin <= 1e-12 else margin_company_limit / margin,
    }
    portfolio_scale = clamp(min(1.0, *scale_candidates.values()), 0.0, 1.0)
    portfolio_binding = [key for key, value in scale_candidates.items() if value < 1.0 and math.isclose(value, min(scale_candidates.values()), rel_tol=1e-8, abs_tol=1e-10)]
    approved_targets: dict[str, int] = {}
    for contract_id, prior in preliminary.items():
        approved = int(math.copysign(math.floor(abs(prior) * portfolio_scale), prior)) if prior else 0
        expected_target = expected[contract_id]
        if abs(approved) > abs(expected_target) or approved * expected_target < 0:
            raise ValueError("risk review expanded or reversed committee position mandate")
        approved_targets[contract_id] = approved
        if approved != expected_target:
            per_contract_binding[contract_id] = sorted(set(per_contract_binding[contract_id] + portfolio_binding))
    approved_estimates = _risk_estimates(market=market, targets=approved_targets, strategy_type=strategy_type, profile=profile, policy=policy, config=config)
    result = {
        ...
        "companyRiskAppetite": {
            ...
            "risk_appetite_radar": dict(company_risk_appetite["risk_appetite_radar"]),
    ...
    identity = {
        ...
        "field_contract_id": contract["contract_id"],
        "field_contract_hash": assets["oil_short_horizon_risk_contract_hash"],
        "position_mandate_hash": committee_position_mandate["identity"]["result_hash"],
