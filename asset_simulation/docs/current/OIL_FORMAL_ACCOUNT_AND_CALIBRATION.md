...
> 校准 owner：`audit_oil_formal_account_calibration.py` v0.2
...
当前可复算报告使用 Seed 42／2026／7777、三种风格、三档授权和三年路径，共27个情景、1944个账户回合。开局资本为 `institution_organization_v1` 提供的1000万美元；旧30亿美元报告只保留为历史 Git 记录，不再代表当前模型。

| 指标 | 最小 | 中位数 | 最大 |
|---|---:|---:|---:|
| CAGR | 0.86% | 11.50% | 27.87% |
| 年化波动 | 2.36% | 5.95% | 14.40% |
| 最大回撤 | −9.38% | −3.01% | −1.34% |
| 半月95% ES | −3.42% | −1.74% | −0.81% |
| 初始保证金／权益最大值 | 2.73% | 11.36% | 20.92% |
| 执行成本／成交名义额 | 2.16bp | 3.04bp | 3.50bp |

结果表明1000万美元自营盘在当前授权范围内可正常运行：35%／85%授权组年化波动中位数为3.55%／8.69%，27个情景没有追保、强平或破产。小体量更少触及全球市场硬容量，因此收益与波动高于旧大体量基线，这是尺度非线性而不是等比例换算。

9组高低授权配对中，9组高授权执行成本 bps 不下降，但只有2组出现单位授权收益衰减（22.22%）。组织契约已明确声明初始小型自营盘不预期触及全球市场容量，故容量衰减继续报告但不参与当前现实性总门禁。新尺度的独立留出样本仍是后续校准工作。
