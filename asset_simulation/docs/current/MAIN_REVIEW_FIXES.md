...
- 来源候选分支：`main-review-fixes-candidate`，现已并入 main。
- 纳入独立定价实验的核心、配置、测试与审计工具；不合并全球船池、垄断经营或成本实验。
...
- 由年度收盘锚插值的原油月线只发布初始年或已完成年度；未结算年度的月线即使月份已经过去也不进入决策快照，因为其数值仍包含未知年末锚。
...
Main CI运行完整Python测试（3.11、3.13）、真实执行的JavaScript参数测试、8 Seed × 60年上游审计，以及5 Seed × 20次年度转移的定价审计。
`tools/run_review_checks.py` 内置最小旧日历公式，复现恒定日需求反例并比较真实Seed旧/新路径；它不读取历史提交、分支或工作树。
上游模型由性质测试和多Seed审计保护，不再永久绑定某个历史 main 的结果哈希，因此后续经审查的模型升级不会被旧快照阻断。
...
实际运行数值将保存在同目录 `REVIEW_FIX_VALIDATION.json`。旧日历适配器只在该自包含验证工具中保留，历史定价实验的振幅结论不再作为已通过的经济校准。
