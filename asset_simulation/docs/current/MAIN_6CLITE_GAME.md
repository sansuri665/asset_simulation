...
航线卡同时显示形成货盘、预计实际装船、未满足货盘、全市场装船截止位，以及本公司报盘队列区间和本公司最末装船位。未满足货盘代表当轮可用船位不足，不会被伪装成已经装出的货量。
...
浏览器不计算货盘、报价、队列、AI或物理结算。所有回合提交和下一回合操作必须带 `round_token`；缺失、旧页面或双击产生的重复令牌都会被后端拒绝。
