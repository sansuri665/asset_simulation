...
> 来源：`route-cargo-haul-reference-review`
> 状态：已写入原油海运 v0.6.9 运行配置
> 参考年：2024  
> 货量单位：百万桶/日（mb/d）  
> 航程单位：海里（nm）
...
本表的 8 项航程修正已经写入 `oil_shipping_demand_v0.6.json`，自原油海运 v0.6.9 起参与每月有效航程和吨海里计算。14 条主要航线参考货量与完整 5×5 货量矩阵保持不变。

界面同时显示基准航程、当月有效航程和按 13 节计算的纯海上航行天数。该天数不含港口等待、装卸或未来船舶层的商业占用时间。
